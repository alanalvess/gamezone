...
export default function GamesXbox() {
        ...
        try {
            await search('/products/all', (products: Product[]) => {
                const filtrados = products.filter(
                    (product) => product.category?.name === "Jogos de Xbox"
                );
                setProducts(filtrados);
            });
        } catch (error) {
            console.error(error);
            alert("Erro ao buscar os jogos de gamesXbox");
        }
    }
            ...
            <div className="flex justify-center w-full">
                {products.length > 0 ? (
                    ...
                    <p className="!pt-5 !pb-10">
                        O estoque de jogos Xbox se esgotou. Volte mais tarde!
                    </p>
                )}
            </div>
        </>
