...
export default function GamesPlaystation() {
        ...
        try {
            await search("/products/all", (products: Product[]) => {
                const playstation = products.filter(
                    (product) => product.category?.name === "Jogos de Playstation"
                );
                setProducts(playstation);
            });
        } catch (error) {
            console.error(error);
            alert("Erro ao buscar jogos de Playstation");
        }
    }
            ...
            <div className="flex justify-center w-full">
                {products.length > 0 ? (
                    ...
                    <p className="!pt-5 !pb-10">
                        O estoque de jogos de Playstation se esgotou. Volte mais tarde!
                    </p>
                )}
            </div>
        </>
